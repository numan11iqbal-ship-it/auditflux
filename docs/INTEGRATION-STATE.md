...
Extension release `5.3.5` widens the working desktop popup to a stable 620px intrinsic width and rebalances the high-width layout. The visible command area can show Images and Schema alongside the primary quick actions, the score categories retain two columns, and the real overview metrics use four readable columns. Chromium action testing confirmed a 620px root and body width with no overflow or runtime exceptions.

Extension release `5.3.6` moves the **H1–H6** overlay control into the top bar and enables it automatically after a successful audit, while keeping the control available to toggle the labels off. The **More** menu is removed. All available real audit sections are now rendered as visible full-width tabs across the widened popup, including issues, headings, links, images, schema, performance, GEO/AEO, accessibility, technical, and resources.

Extension release `5.3.7` hardens the H1–H6 toggle so it removes marker root and style nodes even if a website leaves the page-level overlay state stale. A regression test covers this marker-removal path. The intrinsic popup width is increased to 760px, the section navigation is simplified to labeled visible tabs, and the top bar now includes a **Web App** action that opens the canonical AuditFlux web application without requiring an audit connection. Header labels use Title Case, including **Free**, **Web App**, **Re-Scan**, and **Full Report**.

Extension release `5.3.8` is the acceptance refinement of that control surface. The H1–H6 toggle was exercised in an isolated browser DOM with two actual headings: its first activation created two markers, and its next activation removed all markers as well as the overlay root and style nodes. The intrinsic popup width is now **800px**. The toolbar has more breathing room, while all ten real audit sections are presented in a readable, visible two-row tab grid—without a **More** menu or root horizontal scroll. The remaining visible recovery control is normalized to **Try Again**.

Extension release `5.3.9` corrects both SaaS handoffs. **Web App** now saves and opens the audit for the page currently being inspected when the extension has an active authenticated connection; it otherwise opens the ordinary canonical workspace. **Full Report** always opens the canonical `https://auditflux.vercel.app/audit/:id` URL rather than a configured API deployment alias. This ensures both handoffs use the same origin that holds the user’s existing AuditFlux browser session.

### Unified SaaS report workspace

Release `5.4.0` begins the audited Full Report merge into the existing **single** AuditFlux SaaS workspace. The Web App now supports canonical deep audit routes such as `/audit/:id/overview`, `/audit/:id/issues`, and `/audit/:id/resources`, using the existing sidebar, header, authentication, audit API, and persisted normalized audit record. It adds a Security navigation item; richer Overview, Issues, Headings, Links, Images, Schema, GEO/AEO, Performance, Accessibility, Technical, Security, Resources, HTML Source, History, and Reports/Export surfaces; real-data CSV exports; and Print/Save PDF through the existing browser print flow. Missing saved data is displayed as unavailable rather than synthesized.

The extension **Full Report** button now opens the explicit canonical `/audit/:id/overview` route. The authenticated Web App can use the existing extension bridge to locate saved targets, toggle the already-tested H1–H6 overlay, and request a browser-side **Re-Scan** that runs the deterministic page engine, persists a new audit ID, and preserves historical records. Captured server HTML is now included in the normalized saved audit payload when available, subject to the existing server payload boundary, enabling the integrated Resources/HTML Source view for new audits.

### Export & Report Center and automatic pairing — 5.5.0

Release `5.5.0` replaces the prior customer-managed extension-ID path for the current website experience. The canonical `https://auditflux.vercel.app/connect-extension` route lives inside the existing workspace and pairs an installed extension through the official-origin MV3 content bridge, an authenticated short-lived one-time nonce, an installation-scoped random ID, and a short-lived revocable server session. The member is not shown or asked to copy an extension ID, Vercel value, Supabase credential, access token, or API key. The production connection screen was verified with extension `5.5.0`: it displayed the connected Chrome browser, version, and last-seen time.

The one unified SaaS Report Center is available at `/audit/:id/reports`. Its deep-route fallback bug was repaired and regression-tested. Production verification used real audit `a9f3ea9e-1500-4598-a5e2-dfdf3dda7222` for the Vercel audit page, which displayed the persisted score of 53, 25 issues, and coverage of 39/64. **PDF, XLSX, Complete CSV ZIP, DOCX, standalone HTML, and JSON** all generated successfully from that same saved audit. An Excel cell-length error triggered by captured source evidence was corrected with a lossless **Long Text** worksheet; the repaired XLSX generated successfully in production.

The pairing schema remains additive in the existing dedicated Supabase project. The pairing-session table is explicitly server-only under RLS using a deny-by-default policy. The post-migration advisor check no longer reports a pairing-table policy gap. The remaining Auth advisor concerns optional leaked-password protection, a project-level configuration decision unrelated to extension session access.

### Audit route identity repair — release `b077bd4`

The production workspace now treats the route audit ID as the only valid selected-record identity. Request sequencing rejects stale audit-detail responses, a route change clears previous rendered evidence during loading, and the saved detail response is accepted only when `audit.id` exactly matches the requested route ID. This prevents an older audit response from overwriting a newer explicit deep route.

The existing Git-connected Vercel project automatically deployed commit `b077bd4` as `dpl_4xsGNDGfBdJACGKZJJutF8Kzmn8Y`, which is `READY`. Production same-session navigation from PhotoRoom audit `b40b386f-35a4-487f-8359-b20d39b6772c` to Avnish Parker audit `7755fd51-6148-4fd1-8ce1-e0391055b493` rendered the latter record's URL, 72 score, 49/72 coverage, category evidence, issues, and exact Report Center ID. The focused dashboard suite passed 40 tests; the engine/extension/API suite passed 36 tests; the standalone proxy suite passed 35 tests; TypeScript, Vite production build, and `git diff --check` all passed before release. No new repository, Vercel project, Supabase project, migration, secret, or extension artifact was created.
