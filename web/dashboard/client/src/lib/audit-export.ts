...
import { auditCoverage, auditDetail, categoryContribution, list, number, pageEssentials, record, savedPerformance, securityStatus, sourceStatus, text, type AuditRecord } from './audit-report';
...
function rowsForPageEssentials(report: NormalizedAuditReport) { const item = report.pageEssentials; return [['Audited URL', item.url, item.urlSource, 'available', 'Exact extension location at audit time'], ['Fetched URL', item.fetchedUrl, item.urlSource, item.fetchedUrl ? 'available' : 'unavailable', 'Same-origin browser fetch when available'], ['HTTP Status', item.httpStatus, item.httpStatusSource, item.httpStatusAvailability, item.httpStatusAvailability === 'available' ? 'Observed response status' : 'Not available from browser capture'], ['Indexability', item.indexability, item.indexabilitySource, item.indexability === 'unknown' ? 'unavailable' : 'available', item.indexabilityReason], ['Canonical', item.canonicalUrl, item.canonicalSource, item.canonicalStatus === 'missing' ? 'unavailable' : 'available', item.canonicalEvidence], ['Canonical Status', item.canonicalStatus, item.canonicalSource, item.canonicalStatus === 'missing' ? 'unavailable' : 'available', item.canonicalEvidence], ['Page Type', item.pageType, item.pageTypeConfidence, item.pageType === 'unknown' ? 'unavailable' : 'available', item.pageTypeEvidence]]; }

function reportDatasets(report: NormalizedAuditReport) {
  return {
    '00-page-essentials.csv': { headers: ['Field', 'Value', 'Source', 'Availability', 'Evidence'], rows: rowsForPageEssentials(report) },
...
export function standaloneHtml(report: NormalizedAuditReport) {
  ...
  const essentials = table(['Field', 'Value', 'Source', 'Availability', 'Evidence'], rowsForPageEssentials(report)); const categories = table(['Category', 'Score', 'Passed', 'Applicable', 'Failed', 'Source'], report.categories.map(item => [item.label || item.category_id, item.score, item.passed, item.checks, item.failed, item.source]));
...
export async function exportReport(format: ExportFormat, report: NormalizedAuditReport) {
  ...
  if (format === 'docx') {
    ...
    appendModule('Page Essentials', ['Field', 'Value', 'Source', 'Availability', 'Evidence'], rowsForPageEssentials(report)); appendModule('Score Breakdown', ['Category', 'Score', 'Passed', 'Applicable', 'Failed', 'Source'], report.categories.map(item => [item.label || item.category_id, item.score, item.passed, item.checks, item.failed, item.source]));
  ...
  addModule('Page Essentials', ['Field', 'Value', 'Source', 'Availability', 'Evidence'], rowsForPageEssentials(report)); addModule('Score Breakdown', ['Category', 'Score', 'Passed', 'Applicable', 'Failed', 'Source'], report.categories.map(item => [item.label || item.category_id, item.score, item.passed, item.checks, item.failed, item.source]));
